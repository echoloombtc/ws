...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/kenDevXD/permission/main/access | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/kenDevXD/backupuseranggun.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git remote add origin https://github.com/kenDevXD/backupuseranggun
    git push -f https://SHA256:oOXgMG/zce6oWBv8wkD030jKd8YfyZ+4FlEHeGT0P6E@github.com/kenDevXD/backupuseranggun.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/kenDevXD/backupuseranggun/main/$NameUser/$NameUser.zip"
sleep 1
